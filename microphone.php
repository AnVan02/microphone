    ...
    <script>
        ...
        function initializeReceiver() {
            ...
            app.peer.on('call', call => {
                ...
                call.on('stream', remoteStream => {
                    ...
                    document.getElementById('receiver-initial-view').classList.add('hiidden');
                    document.getElementById('receiver-connected-view').classList.remove('hidden');
                    showStatus('receiver', 'Đã kết nối với diền thoai', 'connected');
        ...
        function connectWebSocketAndMix(remoteStream) {
            navigator.mediaDevices.getUserMedia({
                ...
                // Kết nối WebSocket - ĐÃ SỬA PORT THÀNH 8888
                console.log("🔄 Đang kết nối WebSocket đến ws://localhost:8888...");
                app.ws = new WebSocket("ws://localhost:8888");
        ...
        function handleQRCodeDetected(device, message) {
            ...
            setTimeout(() => {
                ...
                showStatus(device, 'Đã sẵn sàng cho nối mới!', 'connected');
        ...
        function reset() {
            if (isMobile) {
                document.getElementByid('sender-auto-view').classList.add('hidden');
                document.getElementById('sendeer-connected-view').classList.add('hidden');
                document.getElamantById('sendeer-connected-view').classList.add('hidden')
            }
        }
